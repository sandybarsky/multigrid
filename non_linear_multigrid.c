 ...
 *      a PDE, ie Nu = f, you will need to change the routine 'funct' to contain
 *      only f, and remove the while (t<timemax) loop in main.
 *
 *    Debugging. There may well be errors left in the code. Most of the debugging
 *      I've done had to do with solving the eqns improperly based on the initial
 ...
 *
 *    This program has mostly been written by Sandra Barsky.  My understanding 
...
/* --- requesting space for variables ------------------------------*/
  ...
  while(diffmx>tol){
     ...
     * between the current u, and the previous value of u. 
     */
